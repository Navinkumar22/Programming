...
Use the CelestialBody class to the left as the base class. Create the Satellite and Planet classes, both of which are derived classes of CelestialBody. The Satellite class has the attribute string host_planet while the Planet class has the attribute string host_star.
Given Code
Requirements
...
Note that the metrics above are rough estimates only; they may not reflect the objects’ actual measurements.
*/
...
class Satellite : public CelestialBody {
  public:
    Satellite(double s, double m, string c, string n,string k):CelestialBody(s,m,c,n){
      host_planet=k;
    }
...
class Planet : public CelestialBody {
  public:
    Planet(double s, double m, string c, string n,string k):CelestialBody(s,m,c,n){
      host_star=k;
    }
    string GetHostStar(){
      return host_star;
